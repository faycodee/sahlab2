...
const SprachTeil1 = () => {
  ...
  useEffect(() => {
    axios.get(`${API_URL}/${id}`).then((res) => {
      const spData = res.data?.sprachb?.teil1 || [];
      setData(spData);
      setLoading(false);
    });
  }, [id]);
  ...
  const handleReview = () => {
    ...
    setShowReview(true);
    sessionStorage.setItem("spteil1", correctCount); // Save score
  };
  ...
  return (
    <div className="p-6 space-y-10 max-w-4xl mx-auto">
      {data.map((block, bIdx) => (
        <div key={bIdx} className="space-y-8">
          <h1 className="text-xl font-semibold mb-4">
            📝 Sprachbausteine Teil 1 – {block.titel}
          </h1>
      ...
      <div className="max-sm:flex max-sm:flex-col justify-between mt-8">
        <button
          onClick={() => navigate(`/lesen/${id}/teil3`)}
