...
const HorenTeil3 = () => {
  ...
  const handleReview = () => {
    ...
    setShowReview(true);
    sessionStorage.setItem("horen_teil3", correctCount);
  };
  ...
  return (
    <div className="p-6 space-y-10 max-w-4xl mx-auto">
      <h1 className="text-xl font-semibold mb-4">🎧 Hören Teil 3</h1>
      {data.map((frage) => {
      ...
      <div className="flex justify-between mt-8">
        <button
          onClick={() => navigate(`/hören/ergebnis`)}
          className="px-4 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 transition"
        >
          Weiter zu Ergebnis 2 ➡️
        </button>
      </div>
    </div>
  );
